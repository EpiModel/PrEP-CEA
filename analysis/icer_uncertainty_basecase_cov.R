...
for (i in 9:16) {
  dat[[i-8]] <- readRDS(paste0("analysis/data/simdata_basecase_cleaned/simulation_output_par", i, ".rds"))
  strats[i-8] <- paste0(ifelse(dat[[i-8]]$inputs$prep.optim.init == T, "I", "-"),
                        ifelse(dat[[i-8]]$inputs$prep.optim.adhr == T, "A", "-"),
                        ifelse(dat[[i-8]]$inputs$prep.optim.retn == T, "P", "-"))
  raw <- dat[[i-8]]$epi %>%
    select(cuml.init.int.cost.disc, cuml.cost.disc, int.cost.disc, cuml.qaly.disc, num.alive, num.active) %>%
    mutate(Strategy = strats[i-8])
...
all_icer <- readRDS("uncertainty_results_final.RDS")
